...
- **The pkgdown URL is deliberately absent from `DESCRIPTION`.** `_pkgdown.yml` explains why:
  `R CMD check --as-cran` fetches every `DESCRIPTION` URL and would report a 404 while the
  site is unpublished. 🔴 **CI to publish it now exists (2026-08-20).** Once the first deploy
  lands and the site returns 200, add the URL to `DESCRIPTION` — `_pkgdown.yml` says to do it
  in the same commit. Until then `pkgdown::check_pkgdown()` aborts with "URL is missing
  package url", which is accepted deliberately.
