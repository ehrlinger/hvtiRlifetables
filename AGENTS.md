...
- **`main` is protected by a GitHub ruleset, and nothing in this repo records that.** A clone
  ...
  **automatic Copilot code review** on every PR. A rejected push comes from the server, not a
  ...
  `hvtiRtables`, `hvtiRdatasets`, `hvtiBoostmtree` and `temporal_hazard` have
  ...
  `R CMD check` platforms. And **`bypass_actors` is not empty everywhere**: `temporal_hazard`
  and `hvtiRtemplates` grant repository admins `bypass_mode: always`, so on those two the
  maintainer *can* push straight to `main`. On this repository `bypass_actors` is empty and
