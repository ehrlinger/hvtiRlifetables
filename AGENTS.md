...
- **Roxygen markdown IS enabled**: `DESCRIPTION` carries `Roxygen: list(markdown = TRUE)`, so
  backticks, `**bold**` and `[fn()]` links render. Rd macros such as `\code{}` still work, and
  `R/cohort.R` is written in them; do not convert either style to the other for its own sake.
  ⚠️ **Until issue #20 this rule said the opposite, and the sources ignored it.** 129 backtick
  spans, 10 bold spans and 6 dead `[fn()]` cross-references shipped literally in `man/`, past
  `R CMD check`, pkgdown and `docs-current`, because all of it is legal Rd text. The
  `No literal markdown in man/` step in `lint.yaml` is what catches a regression now.
  ⚠️ Sibling packages differ: `hvtiRtemplates` has markdown OFF. Check `DESCRIPTION` before
