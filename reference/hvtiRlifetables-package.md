...
Cleveland Clinic 'SAS' macro `%usmatchd`.

## Details

The macro does not interpolate a life table. It evaluates a stored
three-phase parametric hazard fit on the **age** axis, with time origin
at birth, and reads conditional survival off that one smooth curve
twice:
