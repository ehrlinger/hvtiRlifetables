...
Clinic SAS macro `%usmatchd` evaluates, one record per vintage per
stratum. These are fitted models, not life tables: `%usmatchd` runs
`PROC HAZPRED` against a stored fit on the **age** axis, with time
origin at birth.
...
- vintage:
  ...
  character. One of `"table84"`, `"table2008"`, `"table2023"`. Vintages
  are not interchangeable: model *structure* differs, not only fitted
  values.

- stratum:

  character. One of `"all"`, `"f"`, `"m"`, `"w"`, `"wf"`, `"wm"`, plus
  the vintage's non-white codes – `"o"`, `"of"`, `"om"` for `table84`;
  `"b"`, `"bf"`, `"bm"` for `table2008` and `table2023`.

- params:

  list column of named numeric vectors, length 11: `DELTA`, `THALF`,
  `NU`, `M`, `TAU`, `GAMMA`, `ALPHA`, `ETA`, `E0`, `C0`, `L0`.

- status:

  list column of named integer vectors, length 11. The `_STATUS_` gate.
  `mu = exp(E0 | C0 | L0)` **only** where the corresponding status is
  `1`; status `0` means the phase is absent from the model, not that
  `log(mu)` is zero.

- flags:

  list column of named numeric vectors, length 6: `G1FLAG`, `FIXDEL0`,
  `FIXMNU1`, `G3FLAG`, `FIXGE2`, `FIXGAE2`. These record which sub-form
  was fitted. They are metadata only and are never read during
  evaluation.
...
which reports this per vintage.
