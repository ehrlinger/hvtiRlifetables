...
provenance and race semantics needed to choose one. Vintages are **not**
...
(character identifier), `n_strata` (integer, always 9), `nonwhite_code`
(character, the stratum code this vintage uses for its non-white
category), `nonwhite_meaning` (character, what that category actually
contains), `added` (character date the fits were added, where known,
otherwise `NA`).
